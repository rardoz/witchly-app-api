...
const CLIENT_PRESETS: Record<string, Omit<ClientOptions, 'name' | 'description'>> = {
  admin: {
...
function showUsage() {
  ...
  console.log('');
  console.log('Usage:');
  console.log('  npm run setup:client -- --preset <preset> --name "Client Name"');
  console.log('  npm run setup:client -- --scopes read,write --name "Custom Client"');
  console.log('');
  ...
  console.log('  service    - Service account access (read, write, long-lived)');
  console.log('');
  ...
  console.log('  --force                     Create even if similar client exists');
  console.log('');
  console.log('Examples:');
  console.log('  npm run setup:client -- --preset admin --name "Admin Dashboard"');
  console.log('  npm run setup:client -- --preset read-only --name "Mobile App"');
  console.log('  npm run setup:client -- --scopes read,write --name "Custom API Client" --expires 1800');
}
...
async function parseArguments(): Promise<ClientOptions | null> {
  ...
  if (preset && CLIENT_PRESETS[preset]) {
    ...
    options = {
      ...
      scopes,
      tokenExpiresIn: expiresStr ? parseInt(expiresStr) : 3600,
      type: 'custom'
    };
  } else {
...
async function checkExistingClient(name: string, scopes: string[]): Promise<boolean> {
...
async function createClient(options: ClientOptions) {
  try {
    ...
    console.log(`Token Expires: ${options.tokenExpiresIn} seconds (${Math.round(options.tokenExpiresIn / 3600 * 10) / 10} hours)`);
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('');
