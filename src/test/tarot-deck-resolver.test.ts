...
describe('TarotDeckResolver GraphQL Endpoints', () => {
  ...
  describe('Query: tarotDecksByStatus', () => {
    beforeEach(async () => {
      await TarotDeck.create([
        {
          name: 'Test Active Deck',
          primaryImageUrl: 'https://example.com/active.jpg',
          primaryColor: '#FF5733',
          description: 'An active deck',
          author: 'Test Author',
          status: 'active',
          layoutType: 'default',
          layoutCount: 1,
        ...
        {
          name: 'Test Paused Deck',
          primaryImageUrl: 'https://example.com/paused.jpg',
          primaryColor: '#33FF57',
          description: 'A paused deck',
          author: 'Test Author',
          status: 'paused',
          layoutType: 'default',
          layoutCount: 1,
    ...
    it('should return decks filtered by status', async () => {
      const query = `
        query {
          tarotDecksByStatus(status: "active") {
            id
            name
            status
          }
      ...
      expect(response.status).toBe(200);
      expect(response.body.data.tarotDecksByStatus).toHaveLength(1);
      expect(response.body.data.tarotDecksByStatus[0].status).toBe('active');
    });
  });
});
